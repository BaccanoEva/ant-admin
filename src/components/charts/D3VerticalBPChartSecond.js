...
class D3VerticalBPChartSecond extends React.Component {
    componentDidMount() {
        ...
        [0].forEach(function(i){// 输出两个BP图数据
          ...
          g[i].append("text").attr("x",-50).attr("y",-8).style("text-anchor","middle").text("2016年上");
          g[i].append("text").attr("x", 650).attr("y",-8).style("text-anchor","middle").text("2016年下");
        ...
        chart.append('g')// 输出标题
            .attr('class', 'vertical-bp-chart-second--title')
            .append('text')
            .attr('fill', '#000')
            .attr('font-size', '16px')
    ...
    render() {
        return (
            <div className="vertical-bp-chart-second--sample">
                <svg ref={(r) => this.chartRef = r}></svg>
            </div>
        )
    }
}
D3VerticalBPChartSecond.propTypes = {
...
export default D3VerticalBPChartSecond;