...
class D3VerticalBPChartThird extends React.Component {
    componentDidMount() {
        ...
        [0].forEach(function(i){// 输出两个BP图数据
          ...
          g[i].append("text").attr("x",-50).attr("y",-8).style("text-anchor","middle").text("2016年下");
          g[i].append("text").attr("x", 650).attr("y",-8).style("text-anchor","middle").text("2017年上");
        ...
        chart.append('g')// 输出标题
            .attr('class', 'vertical-bp-chart-third--title')
            .append('text')
            .attr('fill', '#000')
            .attr('font-size', '16px')
    ...
    render() {
        return (
            <div className="vertical-bp-chart-third--sample">
                <svg ref={(r) => this.chartRef = r}></svg>
            </div>
        )
    }
}
D3VerticalBPChartThird.propTypes = {
...
export default D3VerticalBPChartThird;