...
class D3VerticalBPChartForth extends React.Component {
    componentDidMount() {
        ...
        [0].forEach(function(i){// 输出两个BP图数据
          ...
          g[i].append("text").attr("x",-50).attr("y",-8).style("text-anchor","middle").text("2017年上");
          g[i].append("text").attr("x", 650).attr("y",-8).style("text-anchor","middle").text("2017年下");
        ...
        chart.append('g')// 输出标题
            .attr('class', 'vertical-bp-chart-forth--title')
            .append('text')
            .attr('fill', '#000')
            .attr('font-size', '16px')
    ...
    render() {
        return (
            <div className="vertical-bp-chart-forth--sample">
                <svg ref={(r) => this.chartRef = r}></svg>
            </div>
        )
    }
}
D3VerticalBPChartForth.propTypes = {
...
export default D3VerticalBPChartForth;