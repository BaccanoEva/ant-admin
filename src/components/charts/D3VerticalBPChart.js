...
class D3VerticalBPChart extends React.Component {
    componentDidMount() {
        ...
        [0].forEach(function(i){// 输出两个BP图数据
          ...
          g[i].append("text").attr("x",-50).attr("y",-8).style("text-anchor","middle").text("2015年下");
          g[i].append("text").attr("x", 650).attr("y",-8).style("text-anchor","middle").text("2016年上");
